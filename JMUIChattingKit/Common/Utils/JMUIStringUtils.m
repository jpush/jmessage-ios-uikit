...
@implementation JMUIStringUtils {
...
+ (NSString *)getFriendlyDateString:(NSTimeInterval)timeInterval {
  return [JMUIStringUtils getFriendlyDateString:timeInterval forConversation:NO];
}
...
+ (NSString *)getFriendlyDateString:(NSTimeInterval)timeInterval
  ...
  if (theDiff < 60) {
    ...
    if (isTodayYesterday) {
      NSString *todayYesterday = [JMUIStringUtils getTodayYesterdayString:theDate];
      if (isShort) {
      ...
      output = [formatter stringFromDate:theDate];
      if (isPastLong) {
        NSString *thePastDate = [JMUIStringUtils getPastDateString:theDate];
